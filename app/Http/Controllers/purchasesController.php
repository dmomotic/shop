...
class purchasesController extends Controller
{
    public function ObtenerProducto(Request $request){
    	...
    	return view('CompraProducto')->with(compact('products'));
    ...
    */
    public function ComprarProducto(Request $request){
        $r = new Purchase();
        $r->user_id = $request->input('product_id');
        $r->product_id = $request->input('product_id');
        $r->quantity= $request->input('product_id');
        $r->payment= $request->input('product_id');
        $r->save();
        return back();
    }
}
